...
// Zugriffsmethode für die einzige Instanz
    ControllerWrapper& ControllerWrapper::getInstance() {
        static ControllerWrapper instance;   // wird nur einmal erzeugt
    return instance;
    }
    ...
    void ControllerWrapper::begin(){
        ...
        PS4.begin();
        Serial.println("Controller Verbindung gestartet");
        //Debug::getInstance().controller(DebugLevel::Info, "Controller Verbindung gestartet.");
        while (PS4.isConnected() == false) 
        { 
            Serial.println("Controller nicht gefunden.");
            delay(1000);
        }
    ...
    void ControllerWrapper::onDisconnect() { 
